...
//include "insurance-api/otzar/lib/ozar_life.php";
//include "insurance-api/otzar/lib/ozar_health.php";
...
function sogo_vue_load()
{
    wp_enqueue_script('polyfill', get_stylesheet_directory_uri() . '/js/vue/polyfill.min.js');
    wp_enqueue_script('vue', get_stylesheet_directory_uri() . '/js/vue/vue.js');
    wp_enqueue_script('vue-vee', get_stylesheet_directory_uri() . '/js/vue/vee-validate.js', array('vue'));
    wp_enqueue_script('components', get_stylesheet_directory_uri() . '/js/vue/components.js', array('vue-vee'));
    wp_enqueue_script('vue-bootstrap', get_stylesheet_directory_uri() . '/js/vue/bootstrap-vue.min.js', array('vue-vee'));
}
 ...
 *  print select
 */
function sogo_do_select($label, $name, $options_array, $db_array = false, $i = 1, $class = '', $ins_order = false, $array_data = false, $help_array = false)
{
...
function sogo_get_insurance_terms($data)
{
    ...
    $samiCarType = $_POST['levi-code'] > 999999 ? 'מסחרי' : 'פרטי';
...
//	sogo_send_email( 'alex@sogo.co.il, sami@hova.co.il', 'לקוח התעניין בהצעה', $message );
    sogo_send_email('sami@hova.co.il', 'לקוח התעניין בהצעה', $message);
...
function sogo_send_insurance_offer()
{
    ...
    /*$aff = isset($_SESSION['aff']) ? $_SESSION['aff'] : '';
    if(!empty($aff)) {
        $insLink = 'https://'.$_POST['ins_link'].'&aff='.$aff;
    } else {
        $insLink = 'https://'.$_POST['ins_link'];
    }*/
    ...
    $ins_link = isset($_POST['ins_link']) ? $_POST['ins_link'] : '';
    //$leadType = 'הצעה ביטוחים';
    $email = isset($_POST['email']) ? $_POST['email'] : '';
    $phone = isset($_POST['phone']) ? $_POST['phone'] : '';
    $content = isset($_POST['message']) ? $_POST['message'] : '';
    $insuranceDateStart = isset($_POST['insuranceDateStart']) ? $_POST['insuranceDateStart'] : Null;
    $licenseNumber = isset($_POST['licenseNumber']) ? $_POST['licenseNumber'] : Null;
    $type_of_form = isset($_POST['type_of_form']) ? $_POST['type_of_form'] : Null;
    $leadType = isset($_POST['leadType']) ? $_POST['leadType'] : ($type_of_form === 'one' ? 'הצעה למייל' : 'דברו איתי');

    //$leadType = $type_of_form === 'one' ? 'הצעה למייל' : 'דברו איתי';
    $subject = isset($_POST['subjectForLead']) ? $_POST['subjectForLead'] : '';
    ...
    $lidArr = [
        ...
        'extra'                   => implode(",", $_POST),
        'subject'                 => $subject,
        'insurance_select_period' => $insuranceDateStart,
        'license_number'          => $licenseNumber

    ];

    $post = http_build_query($lidArr);
    //$url = "https://api.hova.co.il/api/hova/addLead";

    if ($_SERVER['SERVER_NAME'] === 'stage2.hova.co.il') {
        $url = "http://api-stage.hova.co.il/api/hova/addLead";
    } else {
        $url = "https://api.hova.co.il/api/hova/addLead";
    }
    ...
    if ($type_of_form === 'one') {
        ...
        if (sogo_send_email('sami.henn@gmail.com,sami@hova.co.il', 'לקוח התעניין בהצעה', $message)) {
...
function sogo_store_insurance_params()
{
    ...
    $db_params = get_ins_order($ins_order);// get_option('insurance-order_' . $ins_order, true);

    // check the array merge // kobi
    if (is_array($db_params)) {
        $db_params['order_details'] = array_merge($db_params['order_details'], $insurance_data);
        $db_params[1] = array_merge($db_params['order_details'], $insurance_data);
    } else {
        // for new order that does not have anything in the db
        $db_params = array('order_details', $insurance_data);
    }

//var_dump($db_params);die;

    $_SESSION['ins_orders'][$ins_order]['params'] = $db_params;
    ...
    $db_params['order_details'] = $insurance_data;
    ...
    $result = save_ins_order($ins_order, $db_params);
    // $result = update_option('insurance-order_' . $ins_order, $db_params);

//    echo 'insurance-order_' . $ins_order;
//    var_dump($db_params);die;
    $_SESSION['ins_orders'][$ins_order]['params'] = $db_params;
...
//	}
    ...
    //var_dump($order_data); die(); //kobi print
    ...
    $result = save_ins_order($ins_order, $dbParams);
    ...
    $ai = 0;//id of agent when edit order details
    $oi = 0;//order id
...
//		$paymentInfoArr['cardholder-month']        = '';
        ...
        $paymentInfoArr['card-owner-token'] = ''; //$encryptor->encrypt($order_data['card-number']);  'card-owner-token' -> should be empty for crm view indication (show credit card btn).
...
//var_dump($upsalesArr);die;
//var_dump($order_data);die;
        foreach ($upsalesArr as $upsale => $info) {
            $ind = intval($upsale) + 1;
            if (isset($order_data['use-phone-payment']) && $order_data['use-phone-payment'] === 'on') {
                $info['upsales_payments_num'] = '0';
            } else {
                if ($info['upsales_max_payments_num']) {
                    $info['upsales_payments_num'] = $order_data['upsales_payments_num_' . $ind] + 1 ?? "0";
                } else {
                    $info['upsales_payments_num'] = '0';
                }
            }
    ...
    if (isset($ci) && $ci === 1) {
        $finalInfoArray['insuranceIfo']['ins_order'] = $_SESSION['new-ins_orders'];
    }
    ...
    $finalInfoArray['ai'] = $finalInfoArray['insuranceIfo']['ai'] ? $finalInfoArray['insuranceIfo']['ai'] : $order_data["ai"];
    $finalInfoArray['oi'] = $oi;
    $finalInfoArray['ci'] = $ci;
    $finalInfoArray['ei'] = isset($order_data["ei"]) && !empty($order_data["ei"]) ? $order_data["ei"] : 0;
    $finalInfoArray['currentOrderId'] = isset($order_data["currentOrderId"]) && !empty($order_data["currentOrderId"]) ? $order_data["currentOrderId"] : 0;
    ...
    //   $url = "https://api.hova.co.il/api/hova/addOrder?";
    if ($_SERVER['SERVER_NAME'] === 'stage2.hova.co.il') {
        $url = "https://api-stage.hova.co.il/api/hova/addOrder?";
    } else {
        $url = "https://api.hova.co.il/api/hova/addOrder?";
    }
...
//var_dump($finalOrderArr);die; //kobi

    // send to crm only if we have ins_order : 4/5/2020 (Oren)
    if ($ins_order) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, 1);
        ...
        $order_id = $result->data->row->order->orderId;
...
//	$message .= "</table>";
    ...
    $response = [
        'ins_order' => isset($ci) && $ci === 1 ? $_SESSION['new-ins_orders'] : $ins_order,
        'link'      => $link_page,
        'order_id'  => $order_id,
    ];
...
//	$message .= "</table>";
    ...
    // send email to admin if success and if failed
    sogo_send_email($emails2, ' אישור קליטת הזמנה  באתר חובה ', $message2);

    // send to crm only if we have ins_order and order_id : 4/5/2020 (Oren)
    if ($order_id && $ins_order) {
        ...
        // error in order send to error page
        // redirect user to the error page
        $response = [
            'link'      => site_url('/order-error/'),
            'ins_order' => 'false'
...
function sogo_update_insurance_params()
{
    ...
    $ins_options = get_ins_order($ins_order);//get_option('insurance-order_' . $ins_order);
    ...
    save_ins_order($ins_order, $ins_options);
...
function sogo_get_dira($js = false)
{
...
//
    /**
     *  This section is for dira page
     */
    $use_null = (int)$params->coverage == '80100002';
    $params_convert = array(
        'CoverageGroup'   => (int)$params->coverage,
        'InsuranceDate'   => "1970-01-01T00:00:00.000Z",
        'MAGE'            => $use_null ? null : 80800001,
        'MAGE_PARAMETER'  => $use_null ? null : $params->mage_parameter,// ,
        'MAIM'            => $params->maim,
        'MASHKANTA'       => $use_null ? null : $params->mashkanta,
        'MSUM'            => $use_null ? null : $params->msum,
        'MTVIA'           => $use_null ? null : $params->mtvia,
        'MTYPE'           => (int)$params->mtype,
        'MTYPE_PARAMETER' => $use_null ? null : $params->mtype_parameter,
        'TSUM'            => $params->tsum,
        'TTVIA'           => $params->ttvia,
        'TTYPE'           => $use_null ? null : $params->ttype,
        'TTYPE_PARAMETER' => $use_null ? null : $params->mtype_parameter,


    );
    /**
     * this section is for morgage page
     */
    ...
    $s = in_array((int)$params->mtype, $mtype) ? 7000 : 6000;
    $msum = $params->msum ? $params->msum : $params->house_sm * $s;
    switch ($params->insType) {
        case "1":
            $params_convert = array(
                'CoverageGroup'   => '80100001',
                'InsuranceDate'   => "1970-01-01T00:00:00.000Z",
                'MAGE'            => 80800001,
...
function sogo_get_ozar_params()
{
    ...
    switch ($_GET['class']) {
        case "dira":
            ...
            break;
        case "life":
            $health = new ozar_life();
            break;
        case "health":
            $health = new ozar_health();
            break;
    }
...
add_action('wp_ajax_nopriv_sogo_get_ozar_params', 'sogo_get_ozar_params');
add_action('wp_ajax_sogo_get_ozar_params', 'sogo_get_ozar_params');

function sogo_dira_order()
{
    $order_data = json_decode(stripcslashes($_POST['order_data']), true);
    $company = json_decode($order_data['selectedCompany'], true);

    $full_name = $order_data['fullname'];

    $email = $order_data['email'];
    $phone = isset($order_data['phone']) ? $order_data['phone'] : false;
    $message = recursive_implode($company, '');

    $type_of_form = isset($order_data['type_of_form']) ? $order_data['type_of_form'] : false;

    $message .= "Fullname: $full_name <br/>";
...
//	curl_close( $ch );
    ...
    wp_send_json_success(array('message' => 'email send success', 'type' => 'one', 'link' => 'thx'));


}

add_action('wp_ajax_sogo_dira_order', 'sogo_dira_order');
add_action('wp_ajax_nopriv_sogo_dira_order', 'sogo_dira_order');
...
function sogo_get_life($js = false)
{

    $params = json_decode(stripslashes($_POST['order_data']), true);


    $health = new ozar_life();
    $NumberOfInsured = $params['NumberOfInsured'] == '84200001' ? 1 : 2;
    $params['ListOfInsured'] = array_slice($params['ListOfInsured'], 0, $NumberOfInsured);
    $params['ListOfTracks'] = array_slice($params['ListOfTracks'], 0, $params['NumTracks']);

    foreach ($params['ListOfInsured'] as $key => $item) {
        // cal age
        $today = date("Y-m-d");
        $birthday = DateTime::createFromFormat('d/m/Y', $item['BirthDate']);
        $diff = date_diff($birthday, date_create($today));
        $params['ListOfInsured'][$key]['Age'] = $diff->format('%y');
        $params['ListOfInsured'][$key]['BirthDate'] = $birthday->format("Y-m-d") . "T10:00:00.000Z";
    ...
    $params = array(
        ...
        'NumberOfInsured' => $params['NumberOfInsured'],
        'ListOfInsured'   => $params['ListOfInsured'],
        'ListOfTracks'    => $params['ListOfTracks'],
        'DesiredPeriod'   => $params['DesiredPeriod'] * 12,
        'DesiredSum'      => $params['DesiredSum'],
...
add_action('wp_ajax_nopriv_sogo_get_life', 'sogo_get_life');
add_action('wp_ajax_sogo_get_life', 'sogo_get_life');


function sogo_get_morgage()
{

    $params = json_decode(stripslashes($_POST['order_data']));
    if ($params->insType == 3) {
        $dira = sogo_get_dira(true);
        wp_send_json_success($dira);
    }

    if ($params->insType == 2) {
        $life = sogo_get_life(true);

        wp_send_json_success($life);
    }
    ...
    $life = sogo_get_life(true);
    $return = array();
    foreach ($dira as $key => $item) {
        $found = array_search($item['company'], array_column($life, 'company'));
        if ($found !== false) {
            $dira[$key]['PriceLife'] = $life[$found]['PriceLife'];
            $dira[$key]['Price'] = ceil($dira[$key]['Price'] / 12);
            $dira[$key]['Total'] = $dira[$key]['Price'] + $dira[$key]['PriceLife'];

            $return[] = $dira[$key];
        }
    }
    usort($return, "sogo_sort");
    wp_send_json_success($return);
}

add_action('wp_ajax_nopriv_sogo_get_morgage', 'sogo_get_morgage');
add_action('wp_ajax_sogo_get_morgage', 'sogo_get_morgage');

function sogo_sort($a, $b)
{
    return $a["Total"] > $b["Total"];
}

function sogo_sort_life($a, $b)
{

    return $a["PriceLife"] > $b["PriceLife"];
}

function sogo_get_health()
{
    ...
    $health = new ozar_health();
    $birthday = DateTime::createFromFormat('d/m/Y', $params['Age']);

    $params = array(
        'Age'                    => $birthday->format("Y-m-d") . 'T10:00:00.000Z',
        'Gender'                 => $params['Gender'],
        'SelectedInsuranceTypes' => $params['SelectedInsuranceTypes']

    );

//	$params = array(
//	'Age'                    => '1975-04-02T10:00:00.000Z',
//	'Gender'                 => '83300001',
//	'SelectedInsuranceTypes' => [ "83000001" ]
//
//);
    $health->calc();

    wp_send_json_success($health->connect($params));
}

add_action('wp_ajax_nopriv_sogo_get_health', 'sogo_get_health');
add_action('wp_ajax_sogo_get_health', 'sogo_get_health');

add_action('init', 'sogo_block_user_ins_order');
function sogo_block_user_ins_order()
{
    /*   if (isset($_GET['ins_order'])) {
           session_start();
           if(isset($_SESSION['aiParam']) && $_SESSION['aiParam'] > 0) {
               $order_params = get_option('insurance-order_' . $_GET['ins_order']);//identical-number
           } else {
               if ( $order_params ) {
                   if ( isset( $order_params['identical-number'] ) ) {
                       // THE ORDER IS CLOSED REDIRECT USER
    ...
    if (isset($_GET['ins_order']) && !isset($_SESSION['aiParam']) && !isset($_GET['order_id'])) {
        $order_params = get_ins_order($_GET['ins_order']);//get_option('insurance-order_' . $_GET['ins_order']);//identical-number
        if ($order_params) {
            if (isset($order_params['identical-number'])) {
                // THE ORDER IS CLOSED REDIRECT USER
                wp_redirect(home_url());
        ...
        // if someone get direct to the completed page we send back to home page
    } elseif (($page_slug == 'hova-order-completed' || $page_slug == 'makif-order-completed' || $page_slug == 'zad-g-order-completed') && !isset($_SESSION['aiParam']) && !isset($_GET['order_id'])) {
        wp_redirect(home_url());
        exit();
    }
}
 ...
 * Send Insurance offer to user email
 */
function sogo_send_ozar_offer()
{


    $company = $_POST['selected'];
    ...
    $subject = isset($_POST['email_subject']) ? $_POST['email_subject'] : 'ליד מהאתר ללא נושא';


    $message = recursive_implode($company, '');
    $message .= "שם מלא: $full_name <br/>";
    $message .= "אימייל: $email <br/>";
    $message .= "נייד: $phone <br/>";
...
//	curl_close( $ch );
    ...
    if ($email) {
        //$message      = recursive_implode( $company, '' );
        sogo_send_email($email, 'ההצעה שלך מאתר חובה', $message);
    } else {
        sogo_send_email('oren@sogo.co.il', $subject, $message);
    }

    wp_send_json_success(array('message' => 'email send success', 'link' => $message));

}

add_action('wp_ajax_sogo_send_ozar_offer', 'sogo_send_ozar_offer');
add_action('wp_ajax_nopriv_sogo_send_ozar_offer', 'sogo_send_ozar_offer');
...
//$health->calc();
//
//var_dump( $health->connect( array(
//	'Age'                    => '1980-01-01T10:00:00.000Z',
//	'Gender'                 => '83300001',
//	'SelectedInsuranceTypes' => [ "83000001" ]
//
//) ) );
function sogo_wpcf7_send($wpcf7)
{

    // check if we have a regiser to newsletter and save it to a cookie
    ...
    if (isset($posted_data['subject'])) {
        if (gettype($posted_data['subject']) === 'string') {
            $subject = $posted_data['subject'];
        } else {
            $subject = $posted_data['subject'][0];
        }
    }

    $leadType = '';
    if (isset($posted_data['leadType'])) {
        if (gettype($posted_data['leadType']) === 'string') {
            $leadType = $posted_data['leadType'];
        } else {
            $leadType = $posted_data['leadType'][0];
        }
    }
	
    $row = array(
        'fullName' => isset($posted_data['full_name']) ? $posted_data['full_name'] : '',
        'phone'    => isset($posted_data['phone']) ? $posted_data['phone'] : '',
        'subject'  => $subject,
        'email'    => isset($posted_data['email']) ? $posted_data['email'] : '',
        'content'  => isset($posted_data['message']) ? $posted_data['message'] : '',
        'source'   => $_SERVER['HTTP_REFERER'],
        'name'     => $wpcf7->title(),
        'leadType' => $leadType,
        'license_number' => isset($posted_data['licenseNumber']) ? $posted_data['licenseNumber'] : 0,
        'tz' => isset($posted_data['tz']) ? $posted_data['tz'] : '111111111',
        'extra'    => $extra
    );


    $post = http_build_query($row);
    ...
    //  $redirect = get_field('_sogo_thx_page_default', 'option');
...
add_action('wpcf7_mail_sent', 'sogo_wpcf7_send');


function sogo_clean_step_3($order_params)
{

    $order_params['first-name'] = '';
    $order_params['last-name'] = '';
    $order_params['mobile-phone-number'] = '';
    $order_params['license-number'] = '';
    $order_params['ownership-date'] = '';
    $order_params['identical-number'] = '';
    $order_params['birthday-date'] = '';
    $order_params['gender'] = '';
    $order_params['drive-allowed'] = '';
    $order_params['license-year'] = '';
    $order_params['additional-phone-number'] = '';
    $order_params['email'] = '';
    $order_params['city'] = '';
    $order_params['street'] = '';
    $order_params['house-number'] = '';
    $order_params['apartment-number'] = '';
    $order_params['city-another'] = '';
    $order_params['street-another'] = '';
    $order_params['house-number-another'] = '';
    $order_params['apartment-number-another'] = '';
    $order_params['upsale_1'] = '';
    $order_params['upsale_2'] = '';
    $order_params['upsale_3'] = '';
    $order_params['upsale_4'] = '';
    $order_params['upsale_5'] = '';
    $order_params['upsale_6'] = '';
    $order_params['upsale_7'] = '';
    $order_params['upsale_8'] = '';
    $order_params['mandat-num-payments'] = '';
    $order_params['upsales_payments_num_1'] = '';
    $order_params['upsales_payments_num_2'] = '';
    $order_params['upsales_payments_num_3'] = '';
    $order_params['upsales_payments_num_4'] = '';
    $order_params['upsales_payments_num_5'] = '';
    $order_params['upsales_payments_num_6'] = '';
    $order_params['upsales_payments_num_7'] = '';
    $order_params['upsales_payments_num_8'] = '';
    //var_dump($order_params);die;
    return $order_params;
}


function sogo_request_crm($post, $endpoint)
{

    $url = "https://api.hova.co.il/api/hova/$endpoint?";
    // overwrite if stage system.
    if ($_SERVER['SERVER_NAME'] === 'stage2.hova.co.il') {
        $url = "https://api-stage.hova.co.il/api/hova/$endpoint?";
    }
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_POST, 1);
...
//function sogo_build_rows_from_api_kobi_only($source) // not in use
//{
//    if($source === 'car'){
...
//
//                    $fields_array = ['tozeret_cd', 'degem_cd', 'sug_degem', 'shnat_yitzur', 'mispar_rechev'];
//
...
add_action('wp_ajax_sogo_get_gov_car', 'sogo_get_gov_car');
add_action('wp_ajax_nopriv_sogo_get_gov_car', 'sogo_get_gov_car');

//var_dump(sogo_get_gov_car('3051474'));die;
function sogo_get_gov_car()
{
    $car_no = $_POST['license_num'] ?? false;
    $ans = [];
    if ($car_no) {

        $gov_car_no = sogo_search_gov_car_no($car_no);  // search in api url q
        if (!$gov_car_no) {
            $gov_car_no = sogo_get_gov_car_no_db($car_no); // check in the imported gov license db table
        }
        $ans['car'] = $gov_car_no;
        if ($gov_car_no) {
            $tozeret_cd = intval($gov_car_no->tozeret_cd);
            $degem_cd = intval($gov_car_no->degem_cd);
            $sug_degem = $gov_car_no->sug_degem;
            $shnat_yitzur = intval($gov_car_no->shnat_yitzur);
            $gove_car_info = sogo_get_gov_car_info($tozeret_cd, $degem_cd, $sug_degem, $shnat_yitzur);  // search in api url q by degem_cd
            if (!$gove_car_info) {
                $gove_car_info = sogo_get_gov_car_info_from_db($tozeret_cd, $degem_cd, $sug_degem, $shnat_yitzur); // check in the imported gov model db table
            }
            $ans['model_info'] = $gove_car_info;

            global $wpdb;
            $convert_table = $wpdb->prefix . 'levi_convert';
            $query = "SELECT * FROM {$convert_table} WHERE sug_degem = '{$sug_degem}' and degem_cd = '{$degem_cd}' and tozeret_cd = '{$tozeret_cd}' LIMIT 1";
            $res = $wpdb->get_results($query, ARRAY_A);
            if ($res) {
                $ans['model_from_convert'] = $res[0];
            } else {
                $ans['model_from_convert'] = false;
                global $wpdb;
                $table = $wpdb->prefix . 'licence_number_not_found';
                $data = ['license_number' => $car_no];
                $wpdb->insert($table, $data);
            }
        } else {
            $ans['model_info'] = false;
            $ans['model_from_convert'] = false;
        }
        //var_dump($ans);die;
...
function sogo_search_gov_car_no($car_no)
{
    $car_no = trim(str_replace('-', '', $car_no));
    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL            => "https://data.gov.il/api/3/action/datastore_search?resource_id=053cea08-09bc-40ec-8f7a-156f0677aff3&limit=100&q=$car_no",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING       => "",
        CURLOPT_MAXREDIRS      => 10,
        CURLOPT_TIMEOUT        => 30,
    ...
    if ($err) {
        ...
        // success  - search for the car

        $rows = json_decode($response);

        if ($rows->success && $rows->result->records) {

            foreach ($rows->result->records as $record) {

                if ((string)$record->mispar_rechev === (string)$car_no) {
                    // this is my car = return
                    return $record;
                }
            }
        }
...
function sogo_get_gov_car_no_db($car_no)
{
    global $wpdb;
    $table = $wpdb->prefix . 'gov_license_numbers';
    $query = "SELECT * FROM {$table} WHERE mispar_rechev = '{$car_no}' LIMIT 1";
...
function sogo_get_gov_car_info($tozeret_cd, $degem_cd, $sug_degem, $shnat_yitzur)
{
    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL            => "https://data.gov.il/api/3/action/datastore_search?resource_id=142afde2-6228-49f9-8a29-9b6c3a0cbe40&limit=100&q=$degem_cd",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING       => "",
        CURLOPT_MAXREDIRS      => 10,
        CURLOPT_TIMEOUT        => 30,
    ...
    if ($err) {
        ...
        if ($rows->success && $rows->result->records) {
            ...
            foreach ($rows->result->records as $record) {
                ...
                if ((string)$record->tozeret_cd === (string)$tozeret_cd &&
                    $record->degem_cd === $degem_cd &&
                    $record->sug_degem === $sug_degem &&
                    $record->shnat_yitzur === $shnat_yitzur) {
                    // this is my car info = return
                    return $record;
                }
            }
        }
...
function sogo_get_gov_car_info_from_db($tozeret_cd, $degem_cd, $sug_degem, $shnat_yitzur)
{
    global $wpdb;
    $table = $wpdb->prefix . 'gov_car_models';
    $query = "SELECT * FROM {$table} WHERE sug_degem = '{$sug_degem}' and degem_cd = '{$degem_cd}' and tozeret_cd = '{$tozeret_cd}' and shnat_yitzur = '{$shnat_yitzur}' LIMIT 1";
    $res = $wpdb->get_results($query);
    if ($res) {
        return $res[0];
    } else {
...
add_action('wp_ajax_sogo_load_cities', 'load_cities');
add_action('wp_ajax_nopriv_sogo_load_cities', 'load_cities');

function load_cities()
{
    global $wpdb;
    $query = "SELECT zip_code, city FROM wp_cities  ";
    $result = $wpdb->get_results($query, ARRAY_A);
    wp_send_json_success($result);
}


function get_ins_order($id)
{
    global $wpdb;
 //   $wpdb->show_errors();
    $table = $wpdb->prefix . "ins_orders";
    $sql = "select data from $table where ins_order='$id' limit 1";
...
function save_ins_order($ins_order, $data)
{
    $data = json_encode($data);
    global $wpdb;
 //   $wpdb->show_errors();
    $table = $wpdb->prefix . "ins_orders";
    $sql = "select id from $table where ins_order='$ins_order' limit 1";
    $id = $wpdb->get_var($sql);
    if ($id) {
        $wpdb->update($table, array('data' => $data), array('id' => $id));
    } else {

          $wpdb->insert($table, array('data' => $data, 'ins_order' => $ins_order));
    }
...
// Check connection
    ...
    $sql = "select * from wp_options  WHERE option_name LIKE '%insurance-order%' limit 50000 offset 250000 ";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) > 0) {
        // output data of each row
        while($row = mysqli_fetch_assoc($result)) {
            $ins_order =  str_replace("insurance-order_","",$row['option_name']);
            $data = json_encode(maybe_unserialize($row['option_value']));
